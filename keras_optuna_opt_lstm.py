...
def objective(trial):
    ...
    # optimize multiple Bi-LSTM layers
    for i in range(n_layers):
        # optimize Bi-LSTM parameters
        opt_model.add(Bidirectional(LSTM(units=trial.suggest_categorical(f'units{i}', [x for x in range(8, 400)]),
                                         recurrent_dropout=trial.suggest_float(f'recurrent_dropout{i}', 0.0, 0.7),
